...
def newSpazInit(self, *args, **kwargs):
    ...
    if set_spaz.defaultBoxingGloves: self.equipBoxingGloves()
    ...
    if set_spaz.hp: self._hpText.text = 'HP: ' + str(int(self.hitPoints))
...
def pchandleMessage(self, msg):
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            if set_spaz.nameP: bsUtils.PopupText(msg.powerupType.upper() + "!",
                             color=(1, 1, 1),
                             scale=1.0,
            ...
            elif msg.powerupType == 'impactShower':
            ...
            #     bs.shakeCamera(5)
                ...
                bs.screenMessage('%s called in a drone strike!' %
                                 ...
                                 color=(1, 1, 1))
                bsUtils.PopupText("ATD",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
            ...
            if msg.hitType == 'punch':
                ...
                if damage > 0 and damage < 100:
                    bsUtils.PopupText(u"harder",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                ...
                if damage > 450 and damage < 800:
                    ...
                    bsUtils.PopupText(u"better",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if damage > 801 and damage < 1109:
                    bsUtils.showDamageCount('-' + str(int(damage/10)) + "%", msg.pos, msg.forceDirection)
                    bsUtils.PopupText(u"\ue048GrandMaster\ue048",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if damage > 1110 and damage < 1500:
                    bsUtils.showDamageCount('-' + str(int(damage/10)) + "%", msg.pos, msg.forceDirection)
                    bsUtils.PopupText(u"\ue048BOSSS\ue048",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
